Feature: Speaker infrastructure integration
  As the system
  I want to convert text to spoken audio reliably via offline Piper TTS
  ...
  Scenario: A Spanish phrase returns non-empty MP3 bytes via Piper
    Given the text "hola" and the language code "es"
    When tts_segment calls piper_synthesize
    Then the result is a non-empty audio segment

  Scenario: An English phrase returns non-empty MP3 bytes via Piper
