Feature: Web search tool
  As a user
  I want the assistant to search the web using DuckDuckGo
  So that I get current information without needing API keys

  Scenario: Web search returns formatted results
    Given the DuckDuckGo Lite API returns search results for "rust programming language"
  ...
  Scenario: Web search handles empty results
    Given the DuckDuckGo Lite API returns no results for "xyznonexistent123"
